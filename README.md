...
```bash
go install github.com/nobmurakita/cc-read-ppt@latest
```

ビルド時にバージョンを埋め込む場合:

```bash
go build -ldflags "-X main.version=0.1.0" -o cc-read-ppt .
```
...
```bash
mkdir -p ~/.claude/skills/cc-read-ppt
curl -fsSL https://raw.githubusercontent.com/nobmurakita/cc-read-ppt/main/SKILL.md -o ~/.claude/skills/cc-read-ppt/SKILL.md
```
...
```bash
mkdir -p ~/.claude/skills/cc-read-ppt
cp SKILL.md ~/.claude/skills/cc-read-ppt/SKILL.md
```

インストール後、Claude Code が PowerPoint ファイルの読み取りが必要な場面で自動的に cc-read-ppt を使用する。
...
```bash
cc-read-ppt slides --slide 1 基本設計書.pptx
```
...
```bash
cc-read-ppt slides --slide 2 進捗_20200108.pptx
```
...
```bash
cc-read-ppt slides --slide 7 --extract-images /tmp/images 設計書.pptx
```
...
```bash
cc-read-ppt slides --slide 1 --notes 資料.pptx
```
...
```bash
cc-read-ppt search --text "データ" 基本設計書.pptx
```
