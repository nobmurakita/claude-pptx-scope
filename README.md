...
```bash
curl -fsSL https://raw.githubusercontent.com/nobmurakita/cc-read-pptx/main/install.sh | bash
```

macOS / Linux / Git Bash（Windows）で動作する。インストール後、Claude Code が PowerPoint ファイルの読み取りが必要な場面で自動的に cc-read-pptx を使用する。
