...
```bash
curl -fsSL https://raw.githubusercontent.com/nobmurakita/claude-pptx-scope/main/install.sh | bash
```

macOS / Linux / Git Bash（Windows）で動作する。インストール後、Claude Code が PowerPoint ファイルの読み取りが必要な場面で自動的に pptx-scope を使用する。

### Go ツールとして使う

```bash
go install github.com/nobmurakita/claude-pptx-scope/cmd/pptx-scope@latest
```
...
```bash
pptx-scope slides --slide 1 基本設計書.pptx
```
...
```bash
pptx-scope slides --slide 2 進捗_20200108.pptx
```
...
```bash
pptx-scope slides --slide 7 設計書.pptx
```
...
```bash
pptx-scope slides --slide 1 --notes 資料.pptx
```
...
```bash
pptx-scope image 設計書.pptx ppt/media/image1.png /tmp/img.png
```
...
```bash
pptx-scope search --text "データ" 基本設計書.pptx
```
