...
```jsonl
{"style":1,"name":"メイリオ","size":11,"bold":true,"italic":true,"strikethrough":true,"underline":"single","color":"#FF0000"}
```
...
- プレースホルダー図形では、`name`, `size`, `color` はスライドマスター・レイアウトからの継承値で補完される（後述の「プレースホルダーの継承」を参照）。テーマフォント参照（`+mj-lt` 等）は実フォント名に解決される
...
| その他 | | `font` オブジェクトと同じフィールド（`name`, `size`, `bold`, `color` 等） |
