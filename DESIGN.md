...
3. **`image`** — 画像をファイルに保存する。`slides` 出力の `image_id` を指定して個別に抽出する
...
```json
{
  ...
  "slides": [
    ...
    {"number": 4},
    {"number": 5, "title": "フロー図", "has_images": true, "hidden": true}
  ]
}
```
...
```jsonl
{"slide":1,"title":"基本設計書","shapes":[{"id":1,"type":"rect","placeholder":"ctrTitle","pos":{"x":685800,"y":2286000,"w":7772400,"h":1470025},"z":0,"alignment":{"vertical":"center"},"paragraphs":[{"text":"基本設計書","font":{"name":"メイリオ","size":36,"bold":true,"color":"#333333"},"alignment":{"horizontal":"center"}}]},{"id":2,"type":"rect","placeholder":"subTitle","pos":{"x":1371600,"y":3886200,"w":6400800,"h":1752600},"z":1,"paragraphs":[{"text":"2025年4月版"}]}]}
  ...
  "shapes": [
    {
      ...
      "type": "rect",
      "placeholder": "title",
      "pos": {"x": 457200, "y": 274638, "w": 8229600, "h": 1143000},
      "z": 0,
      "paragraphs": [
        {"text": "目次"}
      ]
    },
    {
      ...
      "type": "rect",
      "placeholder": "body",
      "pos": {"x": 457200, "y": 1600200, "w": 8229600, "h": 4525963},
      "z": 1,
      "paragraphs": [
...
| `pos` | object | 位置とサイズ（`a:xfrm` の `a:off` と `a:ext`。EMU単位） |
...
```json
{"id": 3, "type": "wedgeRoundRectCallout", "pos": {"x": 3000000, "y": 1000000, "w": 2000000, "h": 800000}, "z": 2, "callout_pointer": {"x": 1500000, "y": 2500000}, "paragraphs": [{"text": "ここに注目"}]}
```
...
- 各セルのテキストは `a:txBody` 内の全 `a:t` を結合したプレーンテキスト。セル内の複数段落はスペースで結合する
...
**役割:** 画像をファイルに保存する。`slides` 出力の `image_id`（ZIP内のメディアパス）を指定して、必要な画像だけをオンデマンドで取得する。
...
```bash
cc-read-pptx image example.pptx ppt/media/image1.png /tmp/img.png
```
