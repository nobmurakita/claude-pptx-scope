...
|-----------|-----|------|
| `cols` | number | 列数 |
| `rows` | array | 行の配列。各行はセルテキストの文字列配列 |

- 各セルのテキストは `a:txBody` 内の全 `a:t` を結合したプレーンテキスト。セル内の複数段落は `\n` で結合する
- 結合セル（`gridSpan`, `rowSpan`）は結合元セルのみにテキストを格納し、被結合セルは空文字を出力する
...
- エラーメッセージは stderr に `cc-read-ppt: <メッセージ>` の形式で出力する。stdout には常にJSONのみを出力する
