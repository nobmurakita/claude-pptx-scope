...
| `pos` | object | 位置とサイズ（`a:xfrm` の `a:off` と `a:ext`。EMU単位）。プレースホルダーでスライド上に未指定の場合、レイアウト/マスターから継承する |
...
| `margin_left` | number | 段落の左マージン（EMU単位、`a:pPr` の `marL` 属性）。0または未指定の場合は省略 |
| `indent` | number | 段落の1行目インデント（EMU単位、`a:pPr` の `indent` 属性）。負値でぶら下がりインデント。0または未指定の場合は省略 |
...
- スライド内で2回以上使われるフォント情報は `_styles` に定義を抽出し、段落/リッチテキストランでは `s` フィールドで参照する（後述の「スタイル重複排除」を参照）
...
```json
{"id": 1, "type": "rect", "pos": {"x": 0, "y": 0, "w": 5000000, "h": 500000}, "z": 0, "paragraphs": [{"text": "詳細はこちら", "link": {"url": "https://example.com"}}]}
```
...
```json
{"id": 4, "type": "table", "name": "表 1", "pos": {"x": 457200, "y": 1600200, "w": 8229600, "h": 3000000}, "z": 3, "table": {"cols": 3, "rows": [[{"text": "項目"}, {"text": "説明"}, {"text": "備考"}], [{"text": "機能A", "paragraphs": [{"text": "機能A", "font": {"bold": true}}]}, null, {"text": "必須"}]]}}
```
...
### `pptx-scope image <file> <image_id> [output]`

**役割:** 画像をファイルに保存する。`slides` 出力の `image_id`（ZIP内のメディアパス）を指定して、必要な画像だけをオンデマンドで取得する。`output` を省略すると一時ファイルが自動生成され、パスが stdout に出力される。
...
| `[output]` | 保存先のファイルパス（省略時は一時ファイルを自動生成） |
...
スライド内で同じフォント情報が2回以上使われる場合、スタイル定義を `_styles` 配列に抽出し、段落やリッチテキストランでは `s` フィールドで参照IDを指定する。1回のみ使われるフォントはインラインの `font` のまま残す。
...
スタイル定義はスライド行とは独立したJSONL行として出力される。新規スタイルが初めて使われるスライドの直前に出力される。

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `_styles` | array | スタイル定義の配列 |
| `_styles[]._s` | number | スタイルID（出力全体で一意の連番） |
| `_styles[].その他` | | `font` オブジェクトと同じフィールド（`name`, `size`, `bold`, `color` 等） |

**段落/リッチテキストランの `s` フィールド:**

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `s` | number | `_styles` のスタイルIDへの参照。`font` フィールドの代わりに使用する |

- `s` と `font` は排他的: `s` がある段落/ランには `font` がなく、`font` がある段落/ランには `s` がない
- スライド内でフォント情報が1回のみ使われ、かつスライド横断でも初出の場合は `font` をインラインで出力する
- スタイルIDはスライド横断で共有される（同じフォントは全スライドで同じIDを参照する）
- 新規スタイル定義がないスライドの前には `_styles` 行は出力されない
