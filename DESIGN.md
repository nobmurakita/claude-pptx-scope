...
| `width` | number | 線幅（EMU単位）。`a:ln` の `w` 属性値をそのまま出力 |
...
```json
{"name": "メイリオ", "size": 139700, "bold": true, "italic": true, "strikethrough": true, "underline": "single", "color": "#FF0000"}
```

- デフォルト値のフィールドは省略する
- `size` は EMU単位。hundredths of point（`a:rPr` の `sz` 属性）を EMU に変換（× 127）
...
- 数値はすべてEMU単位で出力する（位置・サイズ・フォントサイズ・線幅。PPTX内部の座標系をそのまま使用し、変換誤差を防ぐ。回転角度のみ度単位）
