...
- 画像は一時ディレクトリに自動抽出される（`image_path` フィールドにパスを出力）
- 書式情報は常に出力する
...
画像（`p:pic`）は `type` が `"picture"` となり、以下の追加フィールドを持つ。画像は一時ディレクトリに自動抽出される。
...
| 埋め込み動画・音声 | メディアファイルの内容はCLIで扱いにくい。画像のみ一時ディレクトリに自動抽出して対応する |
